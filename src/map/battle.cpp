...
static int64 battle_calc_damage_adjustment(struct block_list *src, int64 damage, int flag) {
	unsigned int atk_maps = 0, restricted = 0;
	int rate = 100;
	uint16 attacker = 0;
	...
	map_atk_rate = map_getmapflag_sub(src->m, MF_ATK_RATE, NULL) > 0;
	...
	map_bg = map_getmapflag(src->m, MF_BATTLEGROUND) > 0;
	restricted = map_getmapflag(src->m, MF_RESTRICTED);

	//Checking mapflag
	if ((atk_maps & 1 && (!map_pvp && !map_gvg && !map_bg && !map_atk_rate && !restricted)) ||
		(atk_maps & 2 && map_pvp) ||
		(atk_maps & 4 && map_gvg) ||
		(atk_maps & 8 && map_bg) ||
		(atk_maps & 16 && map_atk_rate) ||
		(restricted && atk_maps & (8 * mapd->zone)))
	{
...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case NPC_VAMPIRE_GIFT:
			...
			break;
		case RK_SONICWAVE:
			skillratio += -100 + (skill_lv + 7) * 100; // ATK = {((Skill Level + 5) x 100) x (1 + [(Caster's Base Level - 100) / 200])} %
		...
		case LG_BANISHINGPOINT:
			...
			break;
		case LG_SHIELDPRESS:
			skillratio += -100 + 150 * skill_lv + (status_get_str(src) + status_get_vit(src)) * 2 / 3;
			if (sd) {
				short index = sd->equip_index[EQI_HAND_L];
		...
		case SR_FALLENEMPIRE:
			...
			skillratio += 250 * skill_lv + status_get_str(src) + status_get_dex(src);
			switch( tstatus->size ) {
				case SZ_SMALL: skillratio += skillratio; // Small
				case SZ_MEDIUM: skillratio += skillratio * 120 / 100; // Medium
				case SZ_BIG: skillratio += skillratio * 140 / 100; // Large
			}
			RE_LVL_DMOD(150);
			break;
		case SR_TIGERCANNON:
		...
		case GN_CART_TORNADO: { // ATK [( Skill Level x 50 ) + ( Cart Weight / ( 150 - Caster Base STR ))] + ( Cart Remodeling Skill Level x 50 )] %
				skillratio += 100 * skill_lv;
				if(sd && sd->cart_weight)
					skillratio += sd->cart_weight / 10 / (120 - min(sd->status.str,120)) + pc_checkskill(sd,GN_REMODELING_CART) * 50;
			}
			break;
		case GN_CARTCANNON:
		...
		case GN_SPORE_EXPLOSION:
			skillratio += 100 + status_get_int(src) + 150 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case GN_WALLOFTHORN:
			skillratio += 10 * skill_lv;
			...
			default: {
				...
				switch(skill_id) {
					...
					case WL_TETRAVORTEX_GROUND:
						...
						else skillratio += 1900 + 200 * skill_lv;
						break;
					...
					case SO_VARETYR_SPEAR: //MATK [{( Endow Tornado skill level x 50 ) + ( Caster INT x Varetyr Spear Skill level )} x Caster Base Level / 100 ] %
						skillratio += -100 + 50 * skill_lv + ((sd) ? pc_checkskill(sd, SO_STRIKING) * 50 : 0);
						skillratio += skillratio - 100 + status_get_int(src) * skill_lv + ((sd) ? pc_checkskill(sd, SA_LIGHTNINGLOADER) * 50 : 0);
						RE_LVL_DMOD(100);
						if (sc && sc->data[SC_BLAST_OPTION])
							skillratio += (sd ? sd->status.job_level * 5 : 0);
						break;
